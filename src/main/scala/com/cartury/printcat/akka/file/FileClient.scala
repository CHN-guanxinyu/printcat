...
object FileClient extends ActorBase("FileClient") {
  ...
  override def sysConfigStr =
    s"""
      |akka.remote.netty.tcp.port=${printcatConfig.FILE_CLIENT_PORT}
    """.stripMargin
}
...
class FileClient(conf: PrintcatConfig) extends Actor with ActorLogging {
  ...
  Future{
    while(true) {
      ...
      if (success) printcatClient ! PrintSuceess(jobId)
      else printcatClient ! PrintError(jobId, msg)
    }
  }
  //(id, success, msg)

  override def receive: Receive = {
    case DownloadFile(jobId, path, f) =>
      if(printcatClient == null) printcatClient = sender
      val request = HttpRequest(
        uri = s"http://${conf.FILE_SERVER_HOST}:${conf.FILE_SERVER_PORT}/file/exchange/${path}0xfff$jobId"
      )
  ...
  def downloadFileTo(jobId: Long, request: HttpRequest, destPath: String, f: File => Unit) = {
    ...
    futResp.andThen {
      ...
      case Success(r @ HttpResponse(code, _, e, _)) =>
        val msg = s"Download request failed, response code: $code ,error: $e"
        log.info(msg)
        r.discardEntityBytes()

      case Success(o) =>
        val msg = s"Unable to download file!$o"
        completedTask.put(PrintResult(jobId, success = false, msg))
        log.info(msg)

      case Failure(err) =>
        val msg = s"Download failed: ${err.getMessage}"
        completedTask.put(PrintResult(jobId, success = false, msg))
        log.info(msg)
    }
  }
